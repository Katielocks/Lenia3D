...
class GLProcessor {
    ...
    resizeFrame(width, height) {
        const gl = this.gl;
        gl.deleteFramebuffer(this.frame.framebuffer);
        gl.deleteTexture(this.frame.texture);
        this.frame = new GLFrameBuffer(gl, width, height, this.options);
        // Update viewport and scissor for default framebuffer
...
export function drawTexture(canvas, texture, options = { format: 'rgba' }) {
    var _a;
    if (((_a = processor === null || processor === void 0 ? void 0 : processor.gl) === null || _a === void 0 ? void 0 : _a.canvas) !== canvas) {
        ...
        const gl = processor.gl;
        // Check if the canvas size has changed
        if (gl.canvas.width !== processor.frame.width || gl.canvas.height !== processor.frame.height) {
            processor.resizeFrame(gl.canvas.width, gl.canvas.height);
        }
    }
